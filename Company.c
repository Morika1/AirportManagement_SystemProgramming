...
void printCompany(const Company* pComp, ...)
{
	...
	va_start(list, pComp);
	const char* next = va_arg(list, char*);
	while (next != NULL)
	{
		printf("_%s", next);
		next = va_arg(list, const char*);
	}
	printf(":\n");
	va_end(list);
	printf("Has %d flights\n", pComp->flightCount);
...
int		saveCompanyToFile(const Company* pComp, const char* fileName)
{
	...
	CHECK_MSG_RETURN_0(fp, Error open copmpany file to write\n);
		...
		//}
	BYTE data[2];
	data[0] = ((pComp->flightCount & 0x1) << 7) | ((pComp->sortOpt & 0x7) << 4) | (strlen(pComp->name) & 0xF);
	data[1] = (pComp->flightCount & 0x1FE) >> 1;
	if (!writeByteToFile(data[0], fp, "Error writing byte to file\n"))
		return 0;
	if (!writeByteToFile(data[1], fp, "Error writing byte to file\n"))
		return 0;
	for (int i = 0; i < strlen(pComp->name) + 1; i++)
	{
		if (!writeByteToFile(pComp->name[i], fp, "Error writing byte to file\n"))
			return 0;
	}
	for (int i = 0; i < pComp->flightCount; i++)
	{
...
int loadCompanyFromFile(Company* pComp, const AirportManager* pManager, const char* fileName)
{
	...
	pComp->flightArr = NULL;
	BYTE data[2];
	if (!readByteFromFile(&data[0], fp, "Error reading byte to file\n"))
		return 0;
	if (!readByteFromFile(&data[1], fp, "Error reading byte to file\n"))
		return 0;
	int len = data[0] & 0xF;
	pComp->sortOpt = (data[0] & 0x70) >> 4;
	pComp->flightCount = (data[1] << 1) | ((data[0] & 0x80) >> 7);
	pComp->name = (char*)malloc(len + 1);
	if (pComp->name == NULL)
		return 0;
	for (int i = 0; i < len + 1; i++)
	{
		if (!readByteFromFile(&pComp->name[i], fp, "Error reading company name\n"))
			return 0;
	}
	...
	if (pComp->flightCount > 0)
	{
		...
		//	return 0;
		//}
		CHECK_NULL_MSG_CLOSE_FILE(pComp->flightArr, Alocation error\n, fp)
	}
	else
		pComp->flightArr = NULL;
